...
const eventCircleSvg = (itemComp, trackRenderingHints) => {
  d3.select(`#${itemComp.htmlId} div svg #circles`).selectAll("circle").data(trackRenderingHints.get(RENDER_HINTS.events)).join("circle").attr("r", 5).attr("cx", d => {
    console.log(d.xPos);
    return d.xPos;
  }).attr("cy", "10%").attr("fill", "white").attr("stroke", "black");
};
...
export class EventlineRenderStrategy extends RenderStrategy {
  ...
  getMappedData(dataElement, trackRenderingHints) {
    return {
      events: trackRenderingHints.get(RENDER_HINTS.events)
    };
  }
